...
def load_label_config(config_path: str) -> GitHubLabelManagementConfig:
    """Load label configuration from YAML file."""
    with open(config_path, 'r') as file:
        return GitHubLabelManagementConfig.serialize(yaml.safe_load(file))


def sync_labels(repo: Repository, label_config: GitHubLabelManagementConfig) -> None:
...
def syncup_as_config() -> None:
    ...
    config = load_label_config('./test/_data/github-labels.yaml')
...
def download_labels(repo: github.Repository) -> None:
    existing_labels: Dict[str, GitHubLabel] = {label.name: label for label in repo.get_labels()}
